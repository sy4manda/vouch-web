// The x402 Cloud handler deployed for every post. src/bankr/deploy.ts replaces the four quoted __PLACEHOLDERS__
...
// to swap and burn. That needs `agentAccess: { enabled: true }` in bankr.x402.json (src/bankr/deploy.ts adds it).
// BUYBACK_MODE=off records the fee and skips swap/burn (hackathon). Default is the keeper
// (src/bankr/buyback.ts, built on src/bankr/keeper.ts), which needs no LLM and reports results back to the database.
