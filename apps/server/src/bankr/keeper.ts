...
//
//   BANKR_API_KEY=... npm --prefix apps/server run keeper
//
// pending.json maps token address -> USDC owed to that token's buyback, e.g. {"0xabc...": 0.35}.
// Run standalone, you fill that file. Inside the backend, src/bankr/buyback.ts drives the same swap/burn calls from the database.
...
// Standalone mode (pending.json). The backend (src/bankr/buyback.ts) imports the functions above instead.
