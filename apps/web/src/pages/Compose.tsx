...
import { FEE_TIERS, MAX_CHARS, MAX_FEE, MIN_FEE } from '@vouch/shared';
          ...
          <div className="tiers" role="radiogroup" aria-label="Fee to unlock">
            ...
            <label className={`tier tier-custom mono${preset === null ? ' on' : ''}${feeError ? ' bad' : ''}`}>
              ...
              <input
                ...
                onChange={(e) => {
                  // digits and one dot, at most three decimals
                  const v = e.target.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1').replace(/(\.\d{3}).+/, '$1');
                  setCustom(v);
                  setPreset(null);
                }}
              />
