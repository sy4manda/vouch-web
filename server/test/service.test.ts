...
import { handlerSource, hookSecret, verifyHookSecret } from '../bankr.ts';
import { keeperTick } from '../buyback.ts';
...
test('createPost validates input', async () => {
  const d = testDeps();
  for (const bad of [{ title: '', text: 'x', feeUsd: 1 }, { title: 't', text: 'x'.repeat(281), feeUsd: 1 }, { title: 't', text: 'x', feeUsd: 0.001 }, { title: 't', text: 'x', feeUsd: 5000 }, { title: 't', text: 'x', feeUsd: 1.234 }]) {
...
test('keeper: swaps owed fees, burns, and never counts its own swap as a vouch', async () => {
  ...
  await keeperTick(d.db, wallet, 0.25, quiet);
...
test('keeper: a failed burn is retried without swapping twice', async () => {
  ...
  await keeperTick(d.db, wallet, 0.25, quiet); // burn still failing: must not swap the new fee on top
  assert.equal(swaps, 1);
  burnFails = false;
  await keeperTick(d.db, wallet, 0.25, quiet); // finishes the stuck burn, then swaps + burns the second fee
