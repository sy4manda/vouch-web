...
import type { CurvePoint, Post, PostDetail, Profile, ProfilePage, Quote, SellQuote, Sort, Voucher } from '../src/lib/types.ts';
import { MAX_CHARS, MAX_FEE, MIN_FEE, TRADE_FEE } from '../src/lib/types.ts';
import type { AuthUser } from './auth.ts';
import { rawToTokens, rawToUsd, tokensToRaw, usdToRaw, type Chain, type ParsedSwap, type PoolRef, type Tx } from './chain.ts';
import type { EndpointDeployer } from './bankr.ts';
import { config } from './config.ts';
import { getMeta, setMeta, tx, type DB } from './db.ts';
...
export type Deps = { db: DB; chain: Chain; deployer: EndpointDeployer };

const CURVE = sampleCurve();
const usdc2 = (n: number) => Math.round(n * 100) / 100;
...
export function validateNewPost(input: { title?: unknown; text?: unknown; feeUsd?: unknown }) {
  ...
  if (!Number.isFinite(fee) || fee < MIN_FEE || fee > MAX_FEE || Math.abs(fee * 100 - Math.round(fee * 100)) > 1e-6) {
    throw bad(`Fee must be between $${MIN_FEE} and $${MAX_FEE} with at most two decimals`);
  }
  return { title, text, feeUsd: usdc2(fee) };
}
...
async function doCreate({ db, chain, deployer }: Deps, user: AuthUser, input: { title?: unknown; text?: unknown; feeUsd?: unknown }): Promise<Post> {
  ...
  try {
    ...
    const launched = await chain.launch({ name: title.slice(0, 32), symbol, tokenURI: endpointUrl, owner: user.wallet as Address });
...
export function recordUnlock(db: DB, o: { postId: string; payer: string; eventId: string }): { newUnlock: boolean; buyback?: Buyback } {
  ...
  return tx(db, () => {
    ...
    db.prepare('INSERT INTO pending_buyback(post_id, usd) VALUES(?,?) ON CONFLICT(post_id) DO UPDATE SET usd = usd + excluded.usd').run(o.postId, row.fee_usd);

    if (config.buybackMode === 'agent') {
      const { usd } = db.prepare('SELECT usd FROM pending_buyback WHERE post_id = ?').get(o.postId) as { usd: number };
      if (usd >= row.fee_usd * config.agentBuybackEvery - 1e-9) {
        ...
        return { newUnlock: inserted, buyback: { token: row.token_address, usdc: usd.toFixed(2) } };
      }
    }
